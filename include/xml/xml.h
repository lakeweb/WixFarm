...
namespace XML
{
	...
	public:
		...
		const pugi::char_t* GetAttribute(const pugi::char_t* name) const { return attribute(name).value(); }
	...
	public:
		...
		//Clear the doc and set the root node; return the root node
		XMLNODE Clear(const char* name = "root") { reset(); return current_node = root().append_child(name); }
		//add a child to this node and return added node.
